...
sudo docker run -d -p 5000:5000 --name hivewing-api --link redis-dev:redis  --link pg-dev:pg --link sqs-dev:sqs --link s3-dev:s3 --env-file container.env -v $CUR_PWD/logs/hivewing-api.log:/home/hivewing/hivewing-api.log hivewing.io/api
#sudo docker run -i -p 5000:5000 --name hivewing-api --link redis-dev:redis  --link pg-dev:pg --link sqs-dev:sqs --link s3-dev:s3 --env-file container.env -v $CUR_PWD/logs:/home/hivewing/logs hivewing.io/api bash
